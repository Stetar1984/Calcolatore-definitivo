...
#==============================================================================
style_ind = {'description_width': 'initial'}; layout_ind = widgets.Layout(width='auto', margin='3px 0')
input_widgets_ind = {
    'nome_ditta': widgets.Text(value='La Mia Ditta Individuale', description="NOME DITTA:", style=style_ind, layout=layout_ind),
    'reddito_simulato_2024': widgets.FloatText(value=70000, description="REDDITO DA SIMULARE O PRESUNTO 2024:", style=style_ind, layout=layout_ind),
    'reddito_rilevante_cpb_2023': widgets.FloatText(value=65000, description="REDDITO RILEVANTE CPB 2023:", style=style_ind, layout=layout_ind),
    'reddito_proposto_cpb_2024': widgets.FloatText(value=72000, description="REDDITO PROPOSTO 2024 AI FINI CPB:", style=style_ind, layout=layout_ind),
    'reddito_impresa_rettificato_cpb': widgets.FloatText(value=72000, description="REDDITO D'IMPRESA RETTIFICATO PER CPB 2024:", style=style_ind, layout=layout_ind),
    'punteggio_isa_n_ind': widgets.FloatSlider(value=8.0, min=1, max=10, step=0.1, description="Punteggio ISA anno n (2023):", style=style_ind, layout=layout_ind),
    'altri_redditi': widgets.FloatText(value=5000, description="ALTRI REDDITI TASSABILI IRPEF 2024:", style=style_ind, layout=layout_ind),
    'oneri_deducibili': widgets.FloatText(value=2000, description="ONERI DEDUCIBILI 2024:", style=style_ind, layout=layout_ind),
...
def esegui_calcolo_individuale(b):
    with output_area_ind:
        ...
        base_imp_no_cpb = val['reddito_simulato_2024'] + val['altri_redditi'] - val['oneri_deducibili'] - val['cedolare_secca_redditi']
        imp_lorda_no_cpb = calcola_irpef(base_imp_no_cpb)
        imp_netta_no_cpb = imp_lorda_no_cpb - val['detrazioni IRPEF']
        imp_finale_no_cpb = (imp_netta_no_cpb if imp_netta_no_cpb > 0 else 0) - val['acconti versati']
        tot_tasse_no_cpb = imp_finale_no_cpb + val['imposta_su_cedolare_secca'] + val['imposte_gia_trattenute']

        # Calcoli CON concordato
        base_imp_si_cpb = val['altri_redditi'] + val['reddito_impresa_rettificato_cpb'] - val['cedolare_secca_redditi'] - val['oneri_deducibili']
        ...
        imp_sostitutiva = base_imponibile_sostitutiva * aliquota_sostitutiva
        imp_lorda_si_cpb = calcola_irpef(base_imp_si_cpb)
        imp_netta_si_cpb = imp_lorda_si_cpb - val['detrazioni IRPEF']
        imp_finale_si_cpb = (imp_netta_si_cpb if imp_netta_si_cpb > 0 else 0) - val['acconti versati']
        tot_tasse_si_cpb = imp_finale_si_cpb + imp_sostitutiva + val['imposta_su_cedolare_secca'] + val['imposte_gia_trattenute']

        risparmio_fiscale = tot_tasse_no_cpb - tot_tasse_si_cpb
        
        display(HTML(f"<h4>Risultati per: {val['nome_ditta']}</h4>")); pd.options.display.float_format = '{:,.2f} €'.format
        df_risultati = pd.DataFrame({
            "Descrizione": ["Base Imponibile IRPEF", "Imposta Lorda", "Imposta Netta", "Imposta a Debito/(Credito)", "Imposta Sostitutiva", "TOTALE CARICO FISCALE"],
            "Senza Concordato": [base_imp_no_cpb, imp_lorda_no_cpb, imp_netta_no_cpb, imp_finale_no_cpb, 0, tot_tasse_no_cpb],
            "Con Concordato": [base_imp_si_cpb, imp_lorda_si_cpb, imp_netta_si_cpb, imp_finale_si_cpb, imp_sostitutiva, tot_tasse_si_cpb]
        }).set_index("Descrizione"); display(df_risultati)
        print("\n" + "-"*65); print(f"<b>RISPARMIO / (MAGGIOR ONERE): {risparmio_fiscale:,.2f} €</b>"); print("="*65)
...
input_societa_widgets = {
    ...
    'reddito_simulato_2024': widgets.FloatText(value=142000.00, description="REDDITO DA SIMULARE O PRESUNTO 2024 (Società):", style=style_soc, layout=layout_largo_soc),
    'valore_produzione_simulato_2024': widgets.FloatText(value=149604.00, description="VALORE PRODUZIONE DA SIMULARE O PRESUNTO 2024 (per IRAP):", style=style_soc, layout=layout_largo_soc),
    'reddito_rilevante_cpb_2023': widgets.FloatText(value=139872.00, description="REDDITO RILEVANTE CPB 2023 (Società):", style=style_soc, layout=layout_largo_soc),
    'reddito_proposto_cpb_2024': widgets.FloatText(value=151784.00, description="REDDITO PROPOSTO 2024 FINI CPB (Società):", style=style_soc, layout=layout_largo_soc),
    'reddito_impresa_rettificato_cpb': widgets.FloatText(value=152420.49, description="REDDITO D'IMPRESA RETTIFICATO PER CPB:", style=style_soc, layout=layout_largo_soc),
    'valore_produzione_irap_rettificato_cpb': widgets.FloatText(value=318210.49, description="Valore della produzione IRAP rettificato per CPB:", style=style_soc, layout=layout_largo_soc),
    'punteggio_isa_n_soc': widgets.FloatSlider(value=8.0, min=1, max=10, step=0.1, description="Punteggio ISA Società (anno n):", style=style_soc, layout=layout_largo_soc)
}
soci_widgets = []
valori_socio1 = {'nome': 'Mario Rossi', 'quota': 50.0, 'altri_redditi': 20000.0, 'oneri_deducibili': 3000.0, 'cedolare_secca_redditi': 0.0, 'imposte_gia_trattenute': 0.0, 'imposta_su_cedolare_secca': 0.0, 'acconti versati': 0.0, 'detrazioni IRPEF': 0.0}
for i in range(4):
